...
function _VersionTitle({ id, badges = [], released }: VersionTitleProps) {
    const anchor = (
        <Anchor to={`/${id}`}>
            <Title>{id}</Title>
        </Anchor>
...
const VersionTitle = memo(_VersionTitle);
...
function VersionEntry({ version, isNewest, isOldest, badges = [], onCopy }: VersionEntryProps) {
    ...
    const versionBadges = useMemo(() => {
        ...
        if (isNewest) {
            newBadges.push(`Newest ${version.type}`);
        }

        if (isOldest) {
            newBadges.push(`Oldest ${version.type}`);
        }
    ...
    return (
        <LazyLoad once>
            <ThemePaper id={version.id}>
                <Stack>
                    <VersionTitle id={version.id} badges={versionBadges} released={version.date.released} />
                    <Group position="apart" sx={{
                        "alignItems": "flex-end"
                    }}>
...
export default function IndexPage() {
    ...
    // TODO: Don't manipulate DOM, rather show/hide
